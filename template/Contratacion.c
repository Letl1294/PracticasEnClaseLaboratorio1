...
int cont_menor(Contratacion* array,int limite)
{
    int i;

    for(i=0;i<limite;i++)
    {
        if((array[i].isEmpty=0) && (array[i].dias<=10))
        {
             printf("[RELEASE] - %d - %s - %s - %d - %d\n",array[i].id, array[i].cuit,array[i].archivo,array[i].dias, array[i].isEmpty);
        }
    }
...
Contratacion* contratacion_getByID(Contratacion array[],int limite,int id)
{
